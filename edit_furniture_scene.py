...
def main():
    ...
    try:
        ...
        df = services.read_edit_excel_file()
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if pd.isna(comment) or "looks good" in str(comment).strip().lower():
                    print("  ⏭️  No action required, skipping...")
                    skipped += 1
                    continue
                ...
                for img_copy_no in range(1,copies + 1):
                    # Step 3: Generate room scene
                    output_filename = f"{wl_model}_room_edit_{img_copy_no}.png"
                    ...
                    df.at[idx, f'Edited Image {img_copy_no}'] = public_url
                
                    print(f"  ✅ SUCCESS! Image URL: {public_url}")

                    if img_copy_no < copies:
                        print("  ⏱️  Waiting 2 seconds before next copy...")
                        time.sleep(2)
        ...
        df.to_excel(config.EXCEL_EDITED_OUTPUT_PATH, index=False)
        print(f"✓ Excel file updated: {config.EXCEL_EDITED_OUTPUT_PATH}")
        ...
        print(f"📁 Output file: {config.EXCEL_EDITED_OUTPUT_PATH}")
        print("=" * 70)
