...
def main():
    ...
    try:
        ...
        vision_client, imagen_model = initialize_google_clients()
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                analysis = analyze_product_image(vision_client, silo_image_url, website_link)
                
                # Step 2: Generate prompt
                prompt = generate_room_scene_prompt(wl_model, analysis)
                
                # Step 3: Generate room scene
                output_filename = f"{wl_model}_room.png"
                local_output_path = output_dir / output_filename
                generate_room_scene(imagen_model, prompt, str(local_output_path))
                
                # Step 4: Upload to SFTP
                public_url = upload_to_sftp(str(local_output_path), output_filename)
